...
class VehicleService {
   ...
   * contract (JSON body, no multipart) is used elsewhere and this avoids
   * changing it. The frontend calls create() then this, one after the
   * other, so it's a single perceived step for the fleet owner even
   * though it's two requests underneath.
   */
  async attachImage(
    id: number,
    userId: number,
